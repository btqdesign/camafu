...
if (!class_exists('MediaDeskClassTopNavigationWalker')) {
	class MediaDeskClassTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				if($item->type_menu == "wide"){
					...
					$menu_type = " wide edgtf-menu-custom-widget";
				} elseif ($item->type_menu == "wide_3_cols"){
					$menu_type = " wide edgtf-menu-3-columns";
				} else {
					$menu_type = " narrow";
				}
			}
			...
			if($item->hide == ""){
                ...
                if($item->featured_icon !== ""){
                    $featured_icon .= sprintf('<i class="edgtf-menu-featured-icon">%s</i>', esc_html__('NEW', 'mediadesk') );
                }
