...
if ( ! function_exists( 'mediadesk_edge_include_title_types' ) ) {
	/**
	 * Load's all title types by going through all folders that are placed directly in title types folder
	 */
	function mediadesk_edge_include_title_types() {
		foreach ( glob( EDGE_FRAMEWORK_MODULES_ROOT_DIR . '/title/types/*/load.php' ) as $module_load ) {
			include_once $module_load;
		}
	}
	
	add_action('mediadesk_edge_action_options_map', 'mediadesk_edge_include_title_types', 1); // 1 is set to just be before title option map init
}

if ( ! function_exists( 'mediadesk_edge_get_title' ) ) {
	/**
...
if ( ! function_exists( 'mediadesk_edge_get_title_holder_classes' ) ) {
	/**
	 * Function that adds classes to title holder div
	 */
	function mediadesk_edge_get_title_holder_classes() {
		$page_id            = mediadesk_edge_get_page_id();
		$title_type_meta    = mediadesk_edge_get_meta_field_intersect( 'title_area_type', $page_id );
		$title_type         = ! empty( $title_type_meta ) ? $title_type_meta : 'standard';
		$title_in_grid_meta = mediadesk_edge_get_meta_field_intersect( 'title_area_in_grid', $page_id );
		$title_img          = mediadesk_edge_get_meta_field_intersect( 'title_area_background_image', $page_id );
		$title_img_behavior = mediadesk_edge_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
		$title_vertical_alignment = mediadesk_edge_get_meta_field_intersect( 'title_area_vertical_alignment', $page_id );
		...
		if ( $title_vertical_alignment === 'bottom' ) {
			$classes[] = 'edgtf-va-bottom';
		}
		
		return implode( ' ', apply_filters( 'mediadesk_edge_filter_title_holder_classes', $classes ) );
	}
}

if ( ! function_exists( 'mediadesk_edge_get_title_holder_styles' ) ) {
	/**
	 * Function that adds inline styles to title holder div
	 */
	function mediadesk_edge_get_title_holder_styles() {
		$page_id               = mediadesk_edge_get_page_id();
		$title_height          = mediadesk_edge_get_title_area_height();
		$title_content_padding = mediadesk_edge_get_title_content_padding();
		$title_bg_color        = mediadesk_edge_get_meta_field_intersect( 'title_area_background_color', $page_id );
		$title_image           = mediadesk_edge_get_meta_field_intersect( 'title_area_background_image', $page_id );
		$title_image_behavior  = mediadesk_edge_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
...
if ( ! function_exists( 'mediadesk_edge_get_title_holder_data' ) ) {
	/**
	 * Function that adds data attributes to title holder div
	 */
	function mediadesk_edge_get_title_holder_data() {
		$page_id            = mediadesk_edge_get_page_id();
		$title_height       = mediadesk_edge_get_title_area_height();
		$title_img          = mediadesk_edge_get_meta_field_intersect( 'title_area_background_image', $page_id );
		$title_img_behavior = mediadesk_edge_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
		...
		if ( ! empty( $title_img ) && $title_img_behavior === 'parallax-zoom-out' ) {
			$attachment_dimensions = mediadesk_edge_get_image_dimensions( $title_img );
		...
		return apply_filters( 'mediadesk_edge_filter_title_holder_data', $data );
	}
}

if ( ! function_exists( 'mediadesk_edge_get_title_wrapper_styles' ) ) {
	/**
	 * Function that adds inline styles to title wrapper div
	 */
	function mediadesk_edge_get_title_wrapper_styles() {
		$page_id                  = mediadesk_edge_get_page_id();
		$title_height             = mediadesk_edge_get_title_area_height();
		$title_content_padding    = mediadesk_edge_get_title_content_padding();
		$title_img_behavior       = mediadesk_edge_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
...
if ( ! function_exists( 'mediadesk_edge_get_title_background_image' ) ) {
	/**
	 * Function that return background image data if background image is set
	 */
	function mediadesk_edge_get_title_background_image() {
		$page_id            = mediadesk_edge_get_page_id();
		$title_img          = mediadesk_edge_get_meta_field_intersect( 'title_area_background_image', $page_id );
		$title_img_behavior = mediadesk_edge_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
		...
		if ( ! empty( $title_img ) && $title_img_behavior !== 'hide' ) {
			$image_id = mediadesk_edge_get_attachment_id_from_url( $title_img );
			$alt      = ! empty( $image_id ) ? get_post_meta( $image_id, '_wp_attachment_image_alt', true ) : '';
			
			$image['src'] = $title_img;
			$image['alt'] = ! empty( $alt ) ? esc_html( $alt ) : esc_html__( 'Image Alt', 'mediadesk' );
		}
...
if ( ! function_exists( 'mediadesk_edge_get_title_area_height' ) ) {
	/**
...
if ( ! function_exists( 'mediadesk_edge_get_title_content_padding' ) ) {
	/**
...
if ( ! function_exists( 'mediadesk_edge_get_title_text' ) ) {
	/**
...
if ( ! function_exists( 'mediadesk_edge_get_title_styles' ) ) {
	/**
	 * Function that adds inline styles to page title
	 */
	function mediadesk_edge_get_title_styles() {
		$page_id = mediadesk_edge_get_page_id();
...
if ( ! function_exists( 'mediadesk_edge_subtitle_text' ) ) {
	/**
	 * Function that echoes subtitle text.
	 */
	function mediadesk_edge_subtitle_text() {
		$subtitle_meta = get_post_meta( mediadesk_edge_get_page_id(), 'edgtf_title_area_subtitle_meta', true );
		$subtitle      = ! empty( $subtitle_meta ) ? $subtitle_meta : '';
		
		return apply_filters( 'mediadesk_edge_filter_subtitle_title_text', $subtitle );
	}
}

if ( ! function_exists( 'mediadesk_edge_get_subtitle_styles' ) ) {
	/**
	 * Function that adds inline styles to page subtitle
	 */
	function mediadesk_edge_get_subtitle_styles() {
		$page_id      = mediadesk_edge_get_page_id();
		...
		if ( $side_padding !== '' ) {
			if ( mediadesk_edge_string_ends_with( $side_padding, '%' ) || mediadesk_edge_string_ends_with( $side_padding, 'px' ) ) {
