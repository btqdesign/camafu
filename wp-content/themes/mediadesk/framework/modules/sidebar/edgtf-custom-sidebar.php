 ...
 */
if ( ! class_exists( 'MediaDeskClassSidebar' ) ) {
	class MediaDeskClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'mediadesk' );
		...
		function template_add_widget_field() {
			$nonce = wp_create_nonce('edgtf-delete-sidebar');
			$nonce = '<input type="hidden" name="edgtf-delete-sidebar" value="' . $nonce . '" />';
			
			echo "\n<script type='text/html' id='edgtf-add-widget'>";
			echo "\n  <form class='edgtf-add-widget' method='POST'>";
			echo "\n  <h3>" . esc_html($this->title) . "</h3>";
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__('Enter Name of the new Widget Area', 'mediadesk') . "' name='edgtf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__('Add Widget Area', 'mediadesk') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'mediadesk_edge_filter_custom_widget_args', $args );
