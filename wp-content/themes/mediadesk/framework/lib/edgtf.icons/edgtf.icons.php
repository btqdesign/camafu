...
/*
  Class: MediaDeskClassIconCollections
...
class MediaDeskClassIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new MediaDeskClassIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new MediaDeskClassIconsFontElegant("Font Elegant", "fe_icon"));
	    $this->addIconCollection('ion_icons', new MediaDeskClassIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new MediaDeskClassLineaIcons('Linea Icons', 'linea_icon'));
    }
    ...
    public function getSocialIconsMetaBoxOrOption($attributes) {
        ...
        if($type == 'meta-box') {
            mediadesk_edge_add_meta_box_field(
                array(
                    ...
                    'label' => $label,
                    'options' => array(
                        'font_awesome'      => esc_html__('Font Awesome', 'mediadesk'),
                        'font_elegant'      => esc_html__('Font Elegant', 'mediadesk'),
                        'ion_icons'         => esc_html__('Ion Icons', 'mediadesk')
                    ),
                    'args' => array(
                        'dependence' => true,
        ...
        foreach ($socialIconCollections as $collection_key => $collection_object) {
            ...
            $edgt_icon_pack_container = mediadesk_edge_add_admin_container(
                array(
                    'parent'			=> $parent,
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'mediadesk'),
        ...
        if(is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__('Icon', 'mediadesk'),
	...
	public function getIconWidgetParamsArray() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'title'   => esc_html__('Icon Pack', 'mediadesk'),
			'name'    => 'icon_pack',
			'options' => array(
				'font_awesome'      => esc_html__('Font Awesome', 'mediadesk'),
				'font_elegant'      => esc_html__('Font Elegant', 'mediadesk'),
				'ion_icons'         => esc_html__('Ion Icons', 'mediadesk'),
				'linea_icons'       => esc_html__('Linea Icons', 'mediadesk')
			)
		);
    ...
    public function getIconCollectionIcons(iMediaDeskInterfaceIconCollection $collection) {
...
if (!function_exists('mediadesk_edge_activate_theme_icons')) {
    function mediadesk_edge_activate_theme_icons() {
        global $mediadesk_edge_global_IconCollections;
        $mediadesk_edge_global_IconCollections = MediaDeskClassIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'mediadesk_edge_activate_theme_icons');
}