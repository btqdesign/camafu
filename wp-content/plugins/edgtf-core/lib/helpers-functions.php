...
if(!function_exists('edgtf_core_get_cpt_shortcode_module_template_part')) {
	 ...
	 * @return html
	 */
	function edgtf_core_get_cpt_shortcode_module_template_part($shortcode, $template, $slug = '', $params = array(), $additional_params = array()) {
...
if(!function_exists('mediadesk_edge_add_user_custom_fields')) {
	/**
	 * Function creates custom social fields for users
	 *
	 * return $user_contact
	 */
	function mediadesk_edge_add_user_custom_fields($user_contact) {
		/**
	...
	add_filter('user_contactmethods', 'mediadesk_edge_add_user_custom_fields');
}
...
if ( ! function_exists( 'edgtf_core_set_open_graph_meta' ) ) {
	...
	function edgtf_core_set_open_graph_meta() {
		...
		if ( mediadesk_edge_option_get_value( 'enable_open_graph' ) === 'yes' ) {
			...
			if ( mediadesk_edge_is_woocommerce_installed() && ( function_exists( 'is_shop' ) && is_shop() ) ) {
			...
			if ( ! empty( $id ) ) {
				...
				if ( get_post_thumbnail_id( $id ) !== '' ) {
					...
					$image = mediadesk_edge_option_get_value( 'open_graph_image' );
				}
			} else {
				global $wp;
	...
	add_action( 'mediadesk_edge_action_header_meta', 'edgtf_core_set_open_graph_meta' );
}