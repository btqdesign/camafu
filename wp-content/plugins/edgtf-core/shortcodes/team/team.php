...
class Team implements lib\ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ( $x = 1; $x < 6; $x ++ ) {
			$teamIconCollections = mediadesk_edge_icon_collections()->getCollectionsWithSocialIcons();
			...
			$team_social_icons_array[] = array(
				...
				'dependency' => array(
					...
					'value'   => mediadesk_edge_icon_collections()->getIconCollectionsKeys()
				)
			);
					...
					'params'                    => array_merge(
						array(
							...
							array(
								...
								'value'       => array_merge( array( '' => '' ), mediadesk_edge_icon_collections()->getIconCollectionsVCExclude( 'linea_icons' ) ),
								'save_always' => true
							),
						),
	...
	public function render( $atts, $content = null ) {
		...
		for ( $x = 1; $x <= $number_of_social_icons; $x ++ ) {
			...
			foreach ( mediadesk_edge_icon_collections()->iconCollections as $collection_key => $collection ) {
	...
	private function getTeamSocialIcons( $params ) {
		...
		if ( $team_social_icon_pack !== '' ) {
			...
			$icon_pack                    = mediadesk_edge_icon_collections()->getIconCollection( $team_social_icon_pack );
			...
			for ( $i = 1; $i <= $params['number_of_social_icons']; $i ++ ) {
				...
				if ( $team_social_icon !== '' ) {
					...
					$social_icons[] = mediadesk_edge_execute_shortcode( 'edgtf_icon', $team_icon_params );
				}
			}
		}
