...
class Separator implements Lib\ShortcodeInterface {
	...
	private function getHolderStyles( $params ) {
		...
		if ( $params['width'] !== '' ) {
			if ( mediadesk_edge_string_ends_with( $params['width'], '%' ) || mediadesk_edge_string_ends_with( $params['width'], 'px' ) ) {
				$styles[] = 'width: ' . $params['width'];
			} else {
				$styles[] = 'width: ' . mediadesk_edge_filter_px( $params['width'] ) . 'px';
			}
		}
		
		if ( $params['thickness'] !== '' ) {
			$styles[] = 'border-bottom-width: ' . mediadesk_edge_filter_px( $params['thickness'] ) . 'px';
		}
		
		if ( $params['top_margin'] !== '' ) {
			if ( mediadesk_edge_string_ends_with( $params['top_margin'], '%' ) || mediadesk_edge_string_ends_with( $params['top_margin'], 'px' ) ) {
				$styles[] = 'margin-top: ' . $params['top_margin'];
			} else {
				$styles[] = 'margin-top: ' . mediadesk_edge_filter_px( $params['top_margin'] ) . 'px';
			}
		}
		
		if ( $params['bottom_margin'] !== '' ) {
			if ( mediadesk_edge_string_ends_with( $params['bottom_margin'], '%' ) || mediadesk_edge_string_ends_with( $params['bottom_margin'], 'px' ) ) {
				$styles[] = 'margin-bottom: ' . $params['bottom_margin'];
			} else {
				$styles[] = 'margin-bottom: ' . mediadesk_edge_filter_px( $params['bottom_margin'] ) . 'px';
			}
		}
