...
if ( ! function_exists( 'edgtf_core_add_admin_shortcodes_styles' ) ) {
	...
	function edgtf_core_add_admin_shortcodes_styles() {
		...
		wp_enqueue_style( 'edgtf_core_vc_shortcodes', EDGE_CORE_ASSETS_URL_PATH . '/css/admin/edgtf-vc-shortcodes.css' );
	}
	
	add_action( 'mediadesk_edge_action_admin_scripts_init', 'edgtf_core_add_admin_shortcodes_styles' );
}
...
if ( ! function_exists( 'edgtf_core_add_admin_shortcodes_custom_styles' ) ) {
	...
	function edgtf_core_add_admin_shortcodes_custom_styles() {
		...
		if ( ! empty( $style ) ) {
			wp_add_inline_style( 'edgtf_core_vc_shortcodes', $style );
		}
	}
	
	add_action( 'mediadesk_edge_action_admin_scripts_init', 'edgtf_core_add_admin_shortcodes_custom_styles' );
}