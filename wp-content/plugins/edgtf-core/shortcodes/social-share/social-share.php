...
class SocialShare implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						array(
							...
							'value'      => array(
								esc_html__( 'List', 'edgtf-core' )     => 'list',
								esc_html__( 'Dropdown', 'edgtf-core' ) => 'dropdown'
							)
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'icon_type',
							'heading'    => esc_html__( 'Icons Type', 'edgtf-core' ),
							'value'      => array(
								esc_html__( 'Font Awesome', 'edgtf-core' ) => 'font-awesome',
								esc_html__( 'Font Elegant', 'edgtf-core' ) => 'font-elegant'
							)
						),
						array(
							'type'       => 'textfield',
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = ( mediadesk_edge_options()->getOptionValue( 'enable_social_share' ) == 'yes' ) ? true : false;
		
		//Is social share enabled for post type
		$post_type         = get_post_type();
		$params['enabled'] = ( mediadesk_edge_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) == 'yes' ) ? true : false;
		
		//Social Networks Data
		$params['networks'] = $this->getSocialNetworksParams( $params );
		
		$params['allow_icon_label'] = $params['allow_icon_label'] === 'yes' ? true : false;
		
		$html = '';
		
		if ( $params['enable_social_share'] ) {
			if ( $params['enabled'] ) {
				$html .= edgtf_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'social-share', '', $params );
			}
		}
	...
	private function getSocialNetworksParams( $params ) {
		...
		$allow_label = $params['allow_icon_label'];
		
		foreach ( $this->socialNetworks as $net ) {
			$html = '';
			
			if ( mediadesk_edge_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( mediadesk_edge_options()->getOptionValue( $net . '_icon' ) ) ? mediadesk_edge_options()->getOptionValue( $net . '_icon' ) : '';
				$params['allow_label'] = $allow_label;
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (mediadesk_edge_options()->getOptionValue('twitter_via') !== '') ? ' via ' . mediadesk_edge_options()->getOptionValue('twitter_via') . ' ' : '';
                if (wp_is_mobile()) {
                    $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(mediadesk_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                } else {
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(mediadesk_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
            case 'google_plus':
            ...
            case 'tumblr':
                ...
                break;
            case 'pinterest':
                $link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . mediadesk_edge_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
                break;
            case 'vk':
