...
if ( ! function_exists( 'mediadesk_edge_get_social_share_html' ) ) {
	/**
	 ...
	 * @return mixed|string
	 */
	function mediadesk_edge_get_social_share_html( $params = array() ) {
		if ( mediadesk_edge_core_plugin_installed() ) {
			return mediadesk_edge_execute_shortcode( 'edgtf_social_share', $params );
		}
	}
}

if ( ! function_exists( 'mediadesk_edge_the_excerpt_max_charlength' ) ) {
	/**
	 ...
	 */
	function mediadesk_edge_the_excerpt_max_charlength( $charlength ) {
		$twitter_via_meta = mediadesk_edge_options()->getOptionValue( 'twitter_via' );
