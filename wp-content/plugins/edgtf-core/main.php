...
if(!function_exists('edgt_core_activation')) {
     ...
     * and defines mediadesk_edge_action_core_on_activate action
     */
    function edgt_core_activation() {
        do_action('mediadesk_edge_action_core_on_activate');
...
if(!function_exists('edgt_core_theme_menu')) {
    ...
    function edgt_core_theme_menu() {
        if (edgtf_core_theme_installed()) {
            ...
            mediadesk_edge_init_theme_options();
            ...
            $page_hook_suffix = add_menu_page(
                ...
                'mediadesk_edge_theme_menu',                // The ID used to bind submenu items to this menu
                array($mediadesk_edge_global_Framework->getSkin(), 'renderOptions'), // The callback function used to render this menu
                $mediadesk_edge_global_Framework->getSkin()->getSkinURI().'/assets/img/admin-logo-icon.png',             // Icon For menu Item
                100           // Position
            );

            foreach ($mediadesk_edge_global_Framework->edgtOptions->adminPages as $key=>$value ) {
                $slug = "";
                ...
                $subpage_hook_suffix = add_submenu_page(
                    'mediadesk_edge_theme_menu',
	                'Edge Options - '.$value->title,                   // The value used to populate the browser's title bar when the menu page is active
                    $value->title,                   // The text of the menu in the administrator's sidebar
                    'administrator',                  // What roles are able to access the menu
                    'mediadesk_edge_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
                    array($mediadesk_edge_global_Framework->getSkin(), 'renderOptions')
                );

                add_action('admin_print_scripts-'.$subpage_hook_suffix, 'mediadesk_edge_enqueue_admin_scripts');
                add_action('admin_print_styles-'.$subpage_hook_suffix, 'mediadesk_edge_enqueue_admin_styles');
            };

            add_action('admin_print_scripts-'.$page_hook_suffix, 'mediadesk_edge_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$page_hook_suffix, 'mediadesk_edge_enqueue_admin_styles');
        }
    }
	...
	function edgt_core_theme_menu_backup_options() {
		if (edgtf_core_theme_installed()) {
			...
			$page_hook_suffix = add_submenu_page(
				...
				'administrator',                  // What roles are able to access the menu
				'mediadesk_edge_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
				array($mediadesk_edge_global_Framework->getSkin(), 'renderBackupOptions')
			);
