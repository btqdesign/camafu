...
abstract class NewsShortcodes {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
				...
				"base"                      => $this->base,
				"category"                  => esc_html__( 'by EDGE NEWS', 'edgtf-news' ),
	...
	public function renderHolders( $atts, $content = null ) {
		...
		$args = edgtf_news_get_shortcode_params_names( $this->getShortcodeParams() );
		...
		$html = '<div ' . mediadesk_edge_get_class_attribute( $holder_classes ) . ' ' . esc_attr( $this->getNewsShortcodesHolderDataParams( $params ) ) . '>';
		...
		$html .= '</div>'; // close edgtf-news-holder
	...
	public function renderQuery( $params, $content = null, $ajax_call = false ) {
		...
		if ( $params['query_result']->have_posts() ) {
			...
			if ( ! $ajax_call ) {
				$html .= '<div ' . mediadesk_edge_get_class_attribute( $inner_classes ) . ' ' .  mediadesk_edge_get_inline_attrs( $inner_data ) . '>';
			}
			...
			while ( $params['query_result']->have_posts() ) : $params['query_result']->the_post();
				...
				if ( $this->isBlockElement() && $post_number == 1 ) {
					$html .= '<div class="edgtf-news-block-part-featured">';
						$html .= $this->render( $params );
					$html .= '</div>';
					$html .= '<div class="edgtf-news-block-part-non-featured">';
				} else {
					$html .= $this->render( $params );
				}
			endwhile;
			...
			if ( $this->isBlockElement() ) {
				$html .= '</div>';  //closing edgtf-news-block-part-non-featured
			}
			if ( ! $ajax_call ) {
	...
	public function errorMessage() {
		$errorMessage = esc_html__( 'Sorry, no posts matched your criteria.', 'edgtf-news' );;
	...
	public function generatePostsQuery( $params ) {
		$queryResult = edgtf_news_get_query( $params );
	 ...
	 * @return array
	 */
	public function getAdditionalHolderClasses() {
		$classes = array();
	...
	private function getHolderClasses( $params ) {
		...
		$holder_classes[] = 'edgtf-news-holder';
		...
		if ( isset( $params['skin'] ) && $params['skin'] !== '' ) {
			$holder_classes[] = 'edgtf-news-skin-' . $params['skin'];
		}
		
		if ( isset( $params['block_proportion'] ) && $params['block_proportion'] !== '' ) {
			$holder_classes[] = 'edgtf-news-block-pp-' . $params['block_proportion'];
		}
		
		if ( $this->isPaginationEnabled( $params ) ) {
			$holder_classes[] = 'edgtf-news-pag-' . $params['pagination_type'];
		}
		
		if ( isset( $params['column_number'] ) && ! $this->isSlider() ) {
			if ( $params['column_number'] !== '' ) {
				$holder_classes[] = 'edgtf-news-columns-' . $params['column_number'];
			} else {
				$holder_classes[] = 'edgtf-news-columns-3';
			}
		}
		
		if ( isset( $params['space_between_items'] ) && $params['space_between_items'] !== '' ) {
			$holder_classes[] = 'edgtf-' . $params['space_between_items'] . '-space';
		}
		
		$additional_holder_classes = $this->getAdditionalHolderClasses();
	...
	protected function getHolderInnerClasses() {
		...
		$holder_inner_classes[] = 'edgtf-news-list-inner';
		
		if ( !$this->isSlider() ) {
			$holder_inner_classes[] = 'edgtf-outer-space';
		}

		if($this->base == 'edgtf_post_carousel1'){
			$holder_inner_classes[] = 'edgtf-slick-slider';
		}

		else if ($this->isSlider()) {
			$holder_inner_classes[] = 'edgtf-owl-slider';
		}
	...
	protected function getHolderInnerData( $params ) {
		...
		$holder_inner_data['data-enable-autoplay']        = isset( $params['slider_autoplay'] ) && ! empty( $params['slider_autoplay'] ) ? $params['slider_autoplay'] : '';
	...
	public function getShortcodeParams( $exclude_options = array() ) {
		...
		if ( $this->isSlider() ) {
			...
			$params_pagination_array = edgtf_news_get_pagination_shortcode_params( $params_pagination_excluded );
	...
	public function newsShortcodesCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['category_title'] ) > 0 ) ? esc_html__( 'Category', 'edgtf-news' ) . ': ' . $value['category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $category ) ) {
				...
				if ( ! empty( $category_title ) ) {
					$category_title_display = esc_html__( 'Category', 'edgtf-news' ) . ': ' . $category_title;
				}
	...
	public function newsShortcodesTagAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data          = array();
				$data['value'] = $value['slug'];
				$data['label'] = ( ( strlen( $value['tag_title'] ) > 0 ) ? esc_html__( 'Tag', 'edgtf-news' ) . ': ' . $value['tag_title'] : '' );
				$results[]     = $data;
			}
		}
	...
	public function newsShortcodesTagAutocompleteRender( $query ) {
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $tag ) ) {
				...
				$tag_title_display = '';
				if ( ! empty( $tag_title ) ) {
					$tag_title_display = esc_html__( 'Tag', 'edgtf-news' ) . ': ' . $tag_title;
				}
	...
	public function newsShortcodesReactionAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data          = array();
				$data['value'] = $value['slug'];
				$data['label'] = ( ( strlen( $value['reaction_title'] ) > 0 ) ? esc_html__( 'Reaction', 'edgtf-news' ) . ': ' . $value['reaction_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $reaction_term ) ) {
				...
				if ( ! empty( $reaction_title ) ) {
					$reaction_title_display = esc_html__( 'Reaction', 'edgtf-news' ) . ': ' . $reaction_title;
				}
	...
	public function newsShortcodesAuthorAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data          = array();
				$data['value'] = $value['ID'];
				$data['label'] = ( ( strlen( $value['user_nicename'] ) > 0 ) ? esc_html__( 'Author', 'edgtf-news' ) . ': ' . $value['user_nicename'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $author ) ) {
				...
				if ( ! empty( $author_user_nicename ) ) {
					$author_display = esc_html__( 'Author', 'edgtf-news' ) . ': ' . $author_user_nicename;
				}
	...
	public function postIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data          = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'edgtf-news' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'edgtf-news' ) . ': ' . $value['title'] : '' );
				$results[]     = $data;
			}
		}
	...
	private function getPaginationHtml( $params ) {
		...
		$additional_condition = isset( $params['pagination_type'] ) && $params['pagination_type'] ==='simple-link' ? true : $params['query_result']->max_num_pages > 1;
		
		if ( $this->isPaginationEnabled( $params ) && $additional_condition ) {
			$html .= edgtf_news_get_shortcode_inner_template_part( 'pagination/' . $params['pagination_type'], '', $params );
		}
	...
	private function getFilter( $params ) {
		...
		switch ( $params['filter_by'] ) {
			case 'category':
				...
				$filter_array[] = array(
					'name' => esc_html__( 'All', 'edgtf-news' ),
			...
			case 'tag':
				...
				$filter_array[] = array( 'name' => esc_html__( 'All', 'edgtf-news' ), 'slug' => $params['tag'] );
		...
		if ( is_array( $filter_array ) && count( $filter_array ) ) {
			$html .= edgtf_news_get_shortcode_inner_template_part( 'filter', '', $filter_params );
		}
	...
	private function getLayoutTitle( $params ) {
		...
		$layout_title_html .= '<div class="edgtf-news-list-title-holder">';
			$layout_title_html .= '<' . $title_tag . ' class="edgtf-news-layout-title">';
				$layout_title_html .= esc_html( $params['layout_title'] );
			$layout_title_html .= '</' . $title_tag . '>';
		$layout_title_html .= '</div>'; //closing of edgtf-news-list-title-holder
