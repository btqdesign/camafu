...
if ( ! function_exists( 'edgtf_news_include_shortcodes_file' ) ) {
	/**
	 * Loades all shortcodes by going through all folders that are placed directly in shortcodes folder
	 */
	function edgtf_news_include_shortcodes_file() {
		foreach ( glob( EDGE_NEWS_SHORTCODES_PATH . '/*/load.php' ) as $shortcode_load ) {
			include_once $shortcode_load;
		}
		
		do_action( 'edgtf_news_action_include_shortcodes_file' );
	}
	
	add_action( 'init', 'edgtf_news_include_shortcodes_file', 6 ); // permission 6 is set to be before vc_before_init hook that has permission 9
}

if ( ! function_exists( 'edgtf_news_load_shortcodes' ) ) {
	function edgtf_news_load_shortcodes() {
		include_once EDGE_NEWS_LIB_PATH . '/shortcode-loader.php';
		
		EdgefNews\Lib\ShortcodeLoader::getInstance()->load();
	}
	
	add_action( 'init', 'edgtf_news_load_shortcodes', 7 ); // permission 7 is set to be before vc_before_init hook that has permission 9 and after edgtf_core_include_shortcodes_file hook
}

if ( ! function_exists( 'edgtf_news_add_admin_shortcodes_styles' ) ) {
	/**
	 * Function that includes shortcodes core styles for admin
	 */
	function edgtf_news_add_admin_shortcodes_styles() {
		//include shortcode styles for Visual Composer
		wp_enqueue_style( 'edgtf_news_vc_shortcodes', EDGE_NEWS_ASSETS_URL_PATH . '/css/admin/edgtf-vc-shortcodes.css' );
	}
	
	add_action( 'mediadesk_edge_action_admin_scripts_init', 'edgtf_news_add_admin_shortcodes_styles' );
}

if ( ! function_exists( 'edgtf_news_add_admin_shortcodes_custom_styles' ) ) {
	/**
	 * Function that print custom vc shortcodes style
	 */
	function edgtf_news_add_admin_shortcodes_custom_styles() {
		$style                  = apply_filters( 'edgtf_news_filter_add_vc_shortcodes_custom_style', $style = '' );
		$shortcodes_icon_styles = array();
		$shortcode_icon_size    = 32;
		$shortcode_position     = 0;
		
		$shortcodes_icon_class_array = apply_filters( 'edgtf_news_filter_add_vc_shortcodes_custom_icon_class', $shortcodes_icon_class_array = array() );
		...
		if ( ! empty( $shortcodes_icon_class_array ) ) {
			foreach ( $shortcodes_icon_class_array as $shortcode_icon_class ) {
				...
				$shortcodes_icon_styles[] = '.vc_element-icon.extended-custom-news-icon' . esc_attr( $shortcode_icon_class ) . ' {
		...
		if ( ! empty( $style ) ) {
			wp_add_inline_style( 'edgtf_news_vc_shortcodes', $style );
		}
	}
	
	add_action( 'mediadesk_edge_action_admin_scripts_init', 'edgtf_news_add_admin_shortcodes_custom_styles' );
}

if ( ! function_exists( 'edgtf_news_get_query' ) ) {
	/**
	 ...
	 *
	 */
	function edgtf_news_get_query( $params, $return_query = true ) {
		...
		switch ( $params['sort'] ) {
			...
			case 'featured_first':
				...
				$query_array['meta_query'] = array(
					'relation' => 'OR',
					'featured' => array(
						'key'     => 'edgtf_news_post_featured_meta',
						'value'   => 'a',
						'compare' => '>'
					),
					'rest'     => array(
			...
			case 'trending_first':
				...
				$query_array['meta_query'] = array(
					'relation' => 'OR',
					'trending' => array(
						'key'     => 'edgtf_news_post_trending_meta',
						'value'   => 'a',
						'compare' => '>'
					),
					'rest'     => array(
			...
			case 'reactions':
				if ( $params['reaction'] !== '' ) {
					...
					$query_array['meta_query'] = array(
						'relation' => 'OR',
						'featured' => array(
							'key'     => 'edgtf_news_reaction_' . $params['reaction'],
							'value'   => '-1',
							'compare' => '>',
							'type'    => 'NUMERIC'
						),
...
if ( ! function_exists( 'edgtf_news_get_shortcode_params_names' ) ) {
	/**
	 ...
	 *
	 */
	function edgtf_news_get_shortcode_params_names( $params_array ) {
		$params_names = array();
...
if ( ! function_exists( 'edgtf_news_get_holder_data_params' ) ) {
	/**
	 ...
	 *
	 */
	function edgtf_news_get_holder_data_params( $params, $base ) {
		$data_string = '';
		
		$query_result = edgtf_news_get_query( $params, false );
		...
		if ( ! empty( $params['featured_custom_image_width'] ) ) {
			$query_result['featured_custom_image_width'] = $params['featured_custom_image_width'];
		}
		
		if ( ! empty( $params['featured_custom_image_height'] ) ) {
			$query_result['featured_custom_image_height'] = $params['featured_custom_image_height'];
		}
...
if ( ! function_exists( 'edgtf_news_shortcodes_load_more' ) ) {
	function edgtf_news_shortcodes_load_more() {
		$params = array();
		...
		$layout       = str_replace( 'edgtf_', '', $params['layout'] );
		$layout       = str_replace( '_', ' ', $layout );
		$layout       = str_replace( ' ', '', ucwords( $layout ) );
		$layout_class = ucfirst( $layout );
		
		$a = '\EdgefNews\CPT\Shortcodes\\' . $layout_class . '\\' . $layout_class;
...
if ( ! function_exists( 'edgtf_news_shortcodes_filter' ) ) {
	function edgtf_news_shortcodes_filter() {
		$params = array();
		...
		$query_params = edgtf_news_get_query( $params, false );
	...
	add_action( 'wp_ajax_nopriv_edgtf_news_shortcodes_filter', 'edgtf_news_shortcodes_filter' );
	add_action( 'wp_ajax_edgtf_news_shortcodes_filter', 'edgtf_news_shortcodes_filter' );
}