...
class EdgefNewsClassWidgetLayout3 extends EdgefNewsPhpClassWidget {
	public function __construct() {
		parent::__construct(
			'edgtf_layout3_widget',
			esc_html__( 'Edge Layout 3 Widget', 'edgtf-news' ),
			array( 'description' => esc_html__( 'Display a layout 3', 'edgtf-news' ) )
		);
	...
	public function widget( $args, $instance ) {
		...
		echo '<div class="widget edgtf-news-widget edgtf-news-layout3-widget">';
			if ( ! empty( $instance['widget_title'] ) ) {
				echo wp_kses_post( $args['before_title'] ) . esc_html( $instance['widget_title'] ) . wp_kses_post( $args['after_title'] );
			}
			
			echo mediadesk_edge_execute_shortcode( 'edgtf_layout3', $instance ); // XSS OK
		echo '</div>';
	}
}