...
class EdgefNewsClassWidgetLayout6 extends EdgefNewsPhpClassWidget {
	public function __construct() {
		parent::__construct(
			'edgtf_layout6_widget',
			esc_html__( 'Edge Layout 6 Widget', 'edgtf-news' ),
			array( 'description' => esc_html__( 'Display a layout 6', 'edgtf-news' ) )
		);
	...
	public function widget( $args, $instance ) {
		...
		echo '<div class="widget edgtf-news-widget edgtf-news-layout6-widget">';
			if ( ! empty( $instance['widget_title'] ) ) {
				echo wp_kses_post( $args['before_title'] ) . esc_html( $instance['widget_title'] ) . wp_kses_post( $args['after_title'] );
			}
			
			echo mediadesk_edge_execute_shortcode( 'edgtf_layout6', $instance ); // XSS OK
		echo '</div>';
	}
}