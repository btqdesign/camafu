...
namespace EdgefNews\CPT\Shortcodes\VideoBlock1;
...
class VideoBlock1 extends Lib\NewsShortcodes {
	...
	function __construct() {
		$this->base            = 'edgtf_video_block1';
		$this->css_class       = 'edgtf-video-block1';
		$this->shortcode_title = esc_html__( 'Video Block 1', 'edgtf-news' );
	...
	public function render( $atts, $content = null ) {
		...
		$featured_params['video_type']     = get_post_meta( $post_id , "edgtf_video_type_meta", true );
		$featured_params['has_video_link'] = get_post_meta( $post_id, "edgtf_post_video_custom_meta", true ) !== '' || get_post_meta( $post_id, "edgtf_post_video_link_meta", true ) !== '';
		
		if ( $featured_params['video_type'] == 'social_networks' ) {
			$featured_params['video_link'] = get_post_meta( $post_id, "edgtf_post_video_link_meta", true );
		} else {
			$featured_params['video_link']  = get_post_meta( $post_id, "edgtf_post_video_custom_meta", true );
			$featured_params['video_link']  .= '?iframe=true'; //added afterwards to get metadata properly
			$featured_params['video_image'] = get_post_meta( $post_id, "edgtf_post_video_image_meta", true);
		}
		...
		if ( $atts['post_number'] == '1' && $featured_params['has_video_link'] ) {
			...
			$html .= edgtf_news_get_shortcode_module_template_part( 'templates/video-layout2-template', 'video-layout2', '', $featured_params );
		} else {
			//Get HTML from template
			$html .= edgtf_news_get_shortcode_module_template_part( 'templates/layout6-template', 'layout6', '', $params );
		}
